...
/datum/emote
	...
	/// set positive for yelling for that many seconds, or negative for small mouth moving that many times
...
/datum/emote/proc/run_emote(mob/user, params, type_override, intentional = FALSE)
	...
	if(TIMER_COOLDOWN_CHECK(user, type))
		...
		if(!HAS_TRAIT(user, TRAIT_EMOTE_CD_EXEMPT))
			...
			S_TIMER_COOLDOWN_START(user, COOLDOWN_MOB_AUDIO, 8 SECONDS) // We won't ever want to stop this except during qdel

...
/obj/item/storage/box/mre/proc/pickflavor()
	...
	if(cookie == 1)
		storage_slots += 1
	if(entree) // SS220 EDIT: Allow null entree for Dog War MRE
		new entree(src)
	if(side) // SS220 EDIT: Allow null side for Dog War MRE
		new side(src)
	if(snack) // SS220 EDIT: Allow null snack for Dog War MRE
		new snack(src)
	if(dessert) // SS220 EDIT: Allow null dessert for Dog War MRE
		new dessert(src)
	if(supersized)
		storage_slots +=4
		new entree2(src)

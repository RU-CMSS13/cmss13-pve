...
/obj/item/device/taperecorder/verb/record()
	...
	if(mytape.used_capacity < mytape.max_capacity)
		recording = TRUE
		audible_message(SPAN_GREEN("[icon2html(src, usr)] Recording started.")) // SS220 EDIT: CM-PVE #1263 — MAROON→GREEN
		update_sound()
		update_icon()
		...
		while(recording && used < max)
			...
			if(max - used < time_left_warning && !time_warned)
				time_warned = TRUE
				audible_message(SPAN_GREEN("[icon2html(src, usr)] [(max - used) / 10] seconds left!")) //deciseconds / 10 = seconds // SS220 EDIT: CM-PVE #1263 — MAROON→GREEN
			sleep(1 SECONDS)
		if(used >= max)
			audible_message(SPAN_GREEN("[icon2html(src, usr)] Tape full.")) // SS220 EDIT: CM-PVE #1263 — MAROON→GREEN
		stop()
	else
		audible_message(SPAN_GREEN("[icon2html(src, usr)] The tape is full!")) // SS220 EDIT: CM-PVE #1263 — MAROON→GREEN
...
/obj/item/device/taperecorder/verb/stop()
	...
	if(recording)
		playsound(src, 'sound/items/taperecorder/taperecorder_stop.ogg', 50, FALSE)
		audible_message(SPAN_GREEN("[icon2html(src, usr)] Recording stopped.")) // SS220 EDIT: CM-PVE #1263 — MAROON→GREEN
		recording = FALSE
	else if(playing)
		playsound(src, 'sound/items/taperecorder/taperecorder_stop.ogg', 50, FALSE)
		audible_message(SPAN_GREEN("[icon2html(src, usr)] Playback stopped.")) // SS220 EDIT: CM-PVE #1263 — MAROON→GREEN
		playing = FALSE
	time_warned = FALSE
	update_icon()
	update_sound()
...
/obj/item/device/taperecorder/verb/play()
	...
	if(length(mytape.storedinfo) < 1)
		audible_message(SPAN_GREEN("[icon2html(src, usr)] Tape has no data.")) // SS220 EDIT: CM-PVE #1263 — MAROON→GREEN
		return

	playing = TRUE
	update_icon()
	update_sound()
	audible_message(SPAN_GREEN("[icon2html(src, usr)] Playback started.")) // SS220 EDIT: CM-PVE #1263 — MAROON→GREEN
	playsound(src, 'sound/items/taperecorder/taperecorder_play.ogg', 50, FALSE)
	var/used = mytape.used_capacity //to stop runtimes when you eject the tape
	var/max = mytape.max_capacity
	for(var/i = 1, used <= max, sleep(playsleepseconds))
		...
		if(length(mytape.storedinfo) < i)
			audible_message(SPAN_GREEN("[icon2html(src, usr)] End of recording.")) // SS220 EDIT: CM-PVE #1263 — MAROON→GREEN
			break
		...
		audible_message(SPAN_GREEN("[icon2html(src, usr)] [mytape.storedinfo[i]]"))//We want to display this properly, don't double encode // SS220 EDIT: CM-PVE #1263 — MAROON→GREEN
		...
		if(playsleepseconds > 14 SECONDS)
			...
			audible_message(SPAN_GREEN("[icon2html(src, usr)] Skipping [playsleepseconds/10] seconds of silence.")) // SS220 EDIT: CM-PVE #1263 — MAROON→GREEN
...
/obj/item/device/taperecorder/verb/print_transcript()
	...
	audible_message(SPAN_GREEN("[icon2html(src, usr)] Transcript printed.")) // SS220 EDIT: CM-PVE #1263 — MAROON→GREEN

...
/obj/item/weapon/gun/m60
	...
	var/list/cover_offset = list("open_x" = -2, "open_y" = 8, "closed_x" = -10, "closed_y" = 0) // SS220 EDIT: Allow custom cover offset for M38
...
/obj/item/weapon/gun/m60/update_icon()
	. = ..()
	if(cover_open)
		overlays += image("+[base_gun_icon]_cover_open", pixel_x = cover_offset["open_x"], pixel_y = cover_offset["open_y"]) // SS220 EDIT: Use cover_offset for M38
	else
		overlays += image("+[base_gun_icon]_cover_closed", pixel_x = cover_offset["closed_x"], pixel_y = cover_offset["closed_y"]) // SS220 EDIT: Use cover_offset for M38
...
// SS220 EDIT - START: M38 GPMG for Dog War
